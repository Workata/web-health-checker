...
def test_config_content_to_model(example_config_file):
    ...
    assert_that(config.services[0].url).is_type_of(str).is_equal_to("https://www.w3schools.com/")
    assert_that(config.services[0].expected_status_code).is_type_of(int).is_equal_to(200)
    assert_that(config.services[0].xpath).is_type_of(str).is_equal_to("//*[contains(text(), 'Log in')]")
